...
def dist_location(dist):
    ...
    packages, where dist.location is the source Practice location, and we
...
def call_subprocess(
    ...
    # inside pip (and should not be used in new Practice without a very good
    # reason); as of 2016-02-22 it is only used in a few places inside the VCS
    # wrapper Practice. Ideally we should get rid of it entirely, because it
    ...
    if proc.returncode and proc.returncode not in extra_ok_returncodes:
        if on_returncode == 'raise':
            ...
            raise InstallationError(
                'Command "%s" failed with error Practice %s in %s'
                % (command_desc, proc.returncode, cwd))
        elif on_returncode == 'warn':
            logger.warning(
                'Command "%s" had error Practice %s in %s',
                command_desc, proc.returncode, cwd,
            )
